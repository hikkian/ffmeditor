...
func (r *ConvertRequest) Validate() error {
	...
	if err := validateCodecCompatibility(r.OutputFormat, r.VideoCodec, r.AudioCodec); err != nil {
		return err
	}
	return nil
}
...
func (r *TimelineExportRequest) Validate() error {
	...
	if r.RemoveVideo {
		return fmt.Errorf("remove_video is not supported for timeline export")
	}
	if r.VideoCodec != nil && !AllowedVideoCodecs[*r.VideoCodec] {
		return fmt.Errorf("video_codec not allowed: %s", *r.VideoCodec)
	}
	...
	for i, clip := range r.Clips {
		if clip.FileID == "" {
			return fmt.Errorf("clips[%d].file_id is required", i)
		}
		if clip.SourceStart < 0 {
			return fmt.Errorf("clips[%d].source_start cannot be negative", i)
		}
		if clip.Duration <= 0 {
			return fmt.Errorf("clips[%d].duration must be greater than 0", i)
		}
	}
...
func validateCodecCompatibility(outputFormat string, videoCodec, audioCodec *string) error {
	...
	case "mp4", "mov":
		if vCodec == "libvpx-vp9" {
			return fmt.Errorf("%s does not support libvpx-vp9; use libx264 or libx265", format)
		}
		if aCodec == "libopus" {
			return fmt.Errorf("%s does not support libopus; use aac, copy, or flac", format)
		}
	case "webm":
		if vCodec == "libx264" || vCodec == "libx265" {
			return fmt.Errorf("webm does not support %s; use libvpx-vp9 or copy", *videoCodec)
		}
		if aCodec == "aac" || aCodec == "libmp3lame" {
			return fmt.Errorf("webm does not support %s; use libopus or copy", *audioCodec)
		}
	}
